...
#
# char 是**俯视游戏里的角色**机位：低仰角 + 小方位角，接近正面站姿。
# 别拿 topdown 渲角色——那是给地面/场景用的。俯视游戏的角色历来是直立精灵，
# 只有地面是俯视的（xianxia-roguelike 现有的 qinglan.png 就是近正面立绘）。
# 用 58° 渲角色会把人压扁成一坨，挥砍那一行基本读不出来。
VIEWS = {
    "audit": (44.0, -8.0, 1.48),
